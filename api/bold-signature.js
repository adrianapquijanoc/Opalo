...
const PRICES = {
  ...
  prueba: 2000 // ⚠️ TEMPORAL — solo para pruebas. Quítalo antes de publicar el sitio real.
};
...
module.exports = async (req, res) => {
  ...
  try {
    const { orderId, currency, items, shipping } = req.body || {};
    ...
    // Guarda el detalle del carrito Y los datos de envío por 24h para que
    // el webhook pueda recuperarlos y armarte el aviso completo (qué compró
    // y a dónde hay que enviarlo).
      ...
      try {
        const payload = JSON.stringify({
          ...
          shipping: shipping && typeof shipping === 'object' ? {
            ...
            cedula: String(shipping.cedula || '').slice(0, 30),
            phone: String(shipping.phone || '').slice(0, 30),
            address: String(shipping.address || '').slice(0, 150),
            city: String(shipping.city || '').slice(0, 60)
          } : null
        });
        const r = await fetch(
          `${redisUrl}/setex/order:${encodeURIComponent(orderId)}/86400/${encodeURIComponent(payload)}`,
