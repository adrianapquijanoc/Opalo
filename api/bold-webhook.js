// ============================================================================
// Webhook: recibe la confirmación de pago de Bold y te avisa por Telegram.
//
// FLUJO:
...
//   4. Enviamos un mensaje directo por Telegram a tu bot.
//
...
//   TELEGRAM_BOT_TOKEN         → el token que te dio @BotFather al crear el bot
//   TELEGRAM_CHAT_ID           → tu chat_id (ver instrucciones para conseguirlo)
//
...
//
// CÓMO CONSEGUIR TELEGRAM_CHAT_ID:
//   1. Busca tu bot en Telegram y mándale cualquier mensaje (ej: "Hola").
//   2. Abre esta URL en tu navegador, reemplazando TU_TOKEN por el token real:
//      https://api.telegram.org/botTU_TOKEN/getUpdates
//   3. Busca en el texto que aparece algo como "chat":{"id":123456789 ...}
//      Ese número (puede ser negativo) es tu TELEGRAM_CHAT_ID.
//
  ...
  try {
    ...
    if (orderId && redisUrl && redisToken) {
      try {
        ...
        if (j && j.result) {
          const raw = decodeURIComponent(j.result);
          try {
            // Formato nuevo: JSON con { description, shipping }
            const parsed = JSON.parse(raw);
            description = parsed.description || description;
            shipping = parsed.shipping || null;
          } catch (parseErr) {
            // Formato viejo: el texto plano de la descripción, sin envío
            description = raw;
          }
        }
      } catch (e) {
    ...
    await sendTelegramNotification({ orderId, totalAmount, paymentMethod, description, shipping });
  } catch (err) {
    console.error('Error procesando el webhook de Bold:', err);
  }
};

async function sendTelegramNotification({ orderId, totalAmount, paymentMethod, description, shipping }) {
  const botToken = cleanEnv(process.env.TELEGRAM_BOT_TOKEN);
  const chatId = cleanEnv(process.env.TELEGRAM_CHAT_ID);

  if (!botToken || !chatId) {
    console.error('Faltan variables de entorno de Telegram (TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID) — no se pudo notificar.');
    return;
  }
  ...
  let text =
    ...
    `🧾 Pedido: \`${orderId || 'sin-referencia'}\`\n` +
    `🛍️ Producto(s): ${description}\n` +
  ...
  if (shipping && (shipping.name || shipping.address)) {
    text +=
      `\n\n📦 *Datos de envío*\n` +
      `👤 ${shipping.name || '—'} (CC ${shipping.cedula || '—'})\n` +
      `📱 ${shipping.phone || '—'}\n` +
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('Telegram respondió con error:', errText);
    }
  } catch (e) {
    console.error('No se pudo enviar la notificación de Telegram:', e);
  }
}
